...
### [2026-05-06] シフト生成時のバグ修正とUI整理

#### 1. 勤務パターンの null 混入防止
- **課題**: 湯本の日曜（営業なし）など、パターンが指定されていない場合に `null` がドロップダウン等に混入して「null:日」と表示されるバグがあった。
- **対応**: `getAvailablePatterns` にて、取得したパターンが `null` および `undefined` の場合は配列から除外するよう修正。

#### 2. 「絶対」ラベルの非表示化
- **課題**: 条件チェックパネルで内部的な優先度判定用タグ「絶対」がすべて画面上に表示され、視認性が低下していた。
- **対応**: `renderConditionsCheck` の描画ロジックにて、タグが「絶対」の場合はHTMLとして描画しないよう修正。スコアリングロジックには影響を与えない。
