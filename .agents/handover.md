...
### ✅ 2026-05-05 条件チェックパネル拡充・CLAUDE.md修正

- `CLAUDE.md`: 「スタッフ11名」→「7名（自動割り当て対象：薬剤師4名・事務3名）」に修正
- `js/generate.js`: `runAllChecks()` に **固定休の遵守チェック** を追加
  - 対象: 鈴木（日・金）/ 服部（月・火）/ 野口（日・月）/ 小野寺（水・土）/ 笠原（火〜土）
  - 仮想固定休は G2（希望休反映）に引っかからないため個別チェックが必要だった
  - 違反1日あたり -200 のペナルティスコアを付与

> 次の一手: `http://localhost:8000/generate.html` でシフト生成→条件チェックパネルを目視確認
